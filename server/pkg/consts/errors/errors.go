...
var (
	...
	ErrIdempotencyKeyAlreadyExists = status.Error(codes.AlreadyExists, "error: idempotency key already exists")
	ErrInvalidPostId               = status.Error(codes.InvalidArgument, "error: invalid post id")
	ErrNoPermissions               = status.Error(codes.PermissionDenied, "error: no permissions")
)
